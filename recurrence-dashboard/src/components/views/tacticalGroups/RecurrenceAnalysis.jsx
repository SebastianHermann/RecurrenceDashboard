...
const RpCard = (props) => {
  ...
  // const [rp, setRpSelection] =  useState(props)
...
export default function RecurrenceAnalysis(props) {
  ...
  const [rpSelection, setRpSelection] = React.useState(
    selectedRP.data ? selectedRP._id.$oid : rps[0]._id.$oid
  );
  ...
  const handleFilterPlayers = (event) => {
    if (event.target.value === 'all') {
      ...
      if (selectedRP.rp_type === 'cross_threshold') {
        console.log('selceted rp type', selectedRP.rp_type);
        console.log('selceted rp type', selectedRP.cross_group_1);
        ...
        console.log('filterTrackingDataY', filterTrackingDataY);
  ...
  useEffect(() => {
    console.log('Game Data X: ', gameDataX);
    console.log('Game Data Y: ', gameDataY);
  }, [gameDataX, gameDataY]);
  ...
  return (
    <React.Fragment>
      <Grid container style={{ background: '#f8f8f8' }} alignItems={'start'}>
        <Grid container item xs={2} style={{ alignContent: 'start' }}>
          <Grid item={12}>
            <List
              dense={true}
              sx={{
        ...
        <Grid container item xs={4} style={{ paddingLeft: '24px' }}>
          <Grid
            item
            xs={12}
            style={{ background: 'white', padding: '0px 24px' }}
          >
            ...
            {selectedRP._id ? (
              <>
                ...
                <Typography
                  ...
                  gutterBottom
                >
                  {selectedRP.rp_type === 'cross_threshold'
                    ? 'Type: ' + selectedRP.rp_type.substring(0, 4) + ' RP'
                    : 'Type: ' + selectedRP.rp_type + ' RP'}
                  <Box
                    component="span"
                    sx={{
        ...
        <Grid container item xs={6} style={{ padding: '0px 24px' }}>
          <Grid
            item
            xs={3}
            style={{
